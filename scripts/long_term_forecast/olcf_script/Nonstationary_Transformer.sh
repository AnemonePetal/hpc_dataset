...
# This script evaluates Nonstationary_Transformer on OLCF dataset for long_term_forecast.
...
MODEL_NAME="Nonstationary_Transformer" # This remains specific to this model's script
...
for PRED_LEN in "${PRED_LENS[@]}"; do
    ...
    COMMAND="python -u run.py --task_name ${TASK_NAME} --is_training ${IS_TRAINING} --root_path ${ROOT_PATH} --data_path ${DATA_PATH} --model_id ${MODEL_ID} --model ${MODEL_NAME} --data ${DATASET} --features ${FEATURES} --seq_len ${SEQ_LEN} --label_len ${LABEL_LEN} --pred_len ${PRED_LEN} --e_layers ${E_LAYERS} --d_layers ${D_LAYERS} --factor ${FACTOR} --enc_in ${ENC_IN} --dec_in ${DEC_IN} --c_out ${C_OUT} --des ${DES} --itr ${ITR} --use_multi_gpu --p_hidden_dims 256 256 --p_hidden_layers 2 --d_model 2048"
